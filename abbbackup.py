...
def generate_business_report(results):
    ...
    if critical_count > 0:
        business_impact_statements.append(f"🚨 {critical_count} critical issues that could be costing you customers right now")
    
    if revenue_potential > 0:
        business_impact_statements.append(f"💰 Potential to increase revenue by ${revenue_potential:.0f}/month")
    
    if traffic_potential > 0:
        business_impact_statements.append(f"📈 Opportunity to attract {traffic_potential} more visitors per month")
    
    if risk_level == 'High':
        business_impact_statements.append("🔴 High security risk - your customers' data may be at risk")
    
    if alt_missing > 0:
        business_impact_statements.append(f"♿ {alt_missing} images missing descriptions - excluding visitors with disabilities")
    
    if cta_count < 3:
        business_impact_statements.append("📢 Few calls-to-action - visitors don't know what to do next")
    
    if trust_signals < 2:
        business_impact_statements.append("🤝 Few trust signals - visitors may not trust your business")
    
    if critical_security:
        business_impact_statements.append(f"🔒 {len(critical_security)} critical security vulnerabilities that put your business at risk")
        ...
        <div class="container">
            ...
            <div class="impact-grid">
                <div class="impact-card">
                    ...
                    <div class="label">Potential Monthly Visitors</div>
                </div>
                <div class="impact-card">
                    <div class="icon">💰</div>
            ...
            <div class="section">
                ...
                <div class="priority-grid">
                    <div class="priority-item priority-critical">
                        ...
                        <div class="label">🚨 Critical - Fix Now</div>
                    </div>
        ...
        for rec in recommendations[:5]:
            ...
            html += f"""
                <div class="issue-item {priority_class}">
                    ...
                    <div class="impact">💡 {business_desc} | Effort: {rec.get('effort', 'Medium')}</div>
                </div>
            """
        html += "</div>"
    ...
    if critical_security:
        html += f"""
            <div class="section" style="background:#f8d7da;">
                ...
                <p style="margin-bottom:10px;">Your website has {len(critical_security)} critical security issues that could:</p>
        ...
        html += """
                ...
                <p style="margin-top:10px;font-weight:bold;">These issues could result in loss of customer trust, data breaches, or legal liability.</p>
            </div>
        """
            ...
            <div class="section" style="background:#e8f4fd;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
                    <div style="background:white;padding:15px;border-radius:8px;text-align:center;">
                        ...
                        <div style="color:#666;">Calls to Action Found</div>
                    </div>
                    <div style="background:white;padding:15px;border-radius:8px;text-align:center;">
                        <div style="font-size:36px;font-weight:bold;">{trust_signals}</div>
                        <div style="color:#666;">Trust Signals Found</div>
                    </div>
                </div>
    """
    
    if cta_count < 3:
        html += '<p style="margin-top:10px;">🟡 Few calls-to-action - visitors may not know what to do next</p>'
    if trust_signals < 2:
        html += '<p style="margin-top:5px;">🟡 Few trust signals - consider adding testimonials and guarantees</p>'
...
def main():
    ...
    if st.sidebar.button("🚀 Start Audit", type="primary"):
        ...
        security = results.get('advanced_security', {})
        if security:
            st.write("✅ Security data found!")  # Temporary debug line
        else:
            st.write("❌ No security data available")  # Temporary debug line
        if security:
            # SSL Certificate
            ssl = security.get('ssl_certificate', {})
            if ssl:
        ...
        if st.session_state.audit_done:
            ...
            # Business Report Download Button
            business_report = generate_business_report(results)
            st.download_button(
                label="📊 Download Business Report (Client-Friendly)",
